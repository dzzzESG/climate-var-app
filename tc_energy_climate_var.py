...
with tab0:
    ...
    if compare_mode and R2:
        sc_color_a  = SC["color"]
        sc_color_b  = SC2["color"]
        sc_label_a  = scenario_name.split(" — ")[0]
        sc_label_b  = scenario_b_name.split(" — ")[0]
        st.markdown(f"""
        <div style="background:var(--bg-card-sel);border:1px solid #3B82F6;border-radius:9px;
          ...
          <span style="color:{sc_color_a};font-weight:700">{sc_label_a}</span>
          &nbsp;vs&nbsp;
          <span style="color:{sc_color_b};font-weight:700">{sc_label_b}</span>
          &nbsp;&mdash;&nbsp;{selected} &nbsp;&middot;&nbsp; {duration}-year horizon &nbsp;&middot;&nbsp; {pass_thru}% pass-through
        </div>""", unsafe_allow_html=True)
    ...
    with dash_right:
        ...
        fig_heat = go.Figure(go.Heatmap(
            ...
            showscale=True,
            colorbar=dict(
                title=dict(text="VaR %", font=dict(size=11, color="#1E293B")),
                thickness=12,
